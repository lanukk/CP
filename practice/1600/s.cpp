	...
	for (int i = 0 ; i < n; i++) {
		for (int j = 0 ; j < n; j++) {
			if (a[i][j] == '.') {
				...
				if (longest_left + longest_right >= 4) {
					possible = 1;
				}
				int up = 0 ;
				for (int k = i - 1; k >= 0; k--) {
				...
				dj = j + 1;
				int lower_right = 0;
				while (di < n && dj < n && a[di][dj] == 'X') {
					lower_right++;
					di++;
					dj++;
				}
				if (lower_right + upper_left >= 4) {
					possible = 1;
				}
				int upper_right = 0 ;
				di =  i - 1;
				dj = j + 1;
				while (di >= 0 && dj < n && a[di][dj] == 'X') {
					di--;
					dj++;
					upper_right++;
				}
				...
				di = i + 1;
				dj = j - 1;
				while (di < n && dj >= 0 && a[di][dj] == 'X') {
					di++;
					dj --;
					lower_left++;
				}
				if (lower_left + upper_right >= 4) {
					possible = 1;
				}
