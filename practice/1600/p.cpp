...
void solve(int input)
{
	...
	while (1) {
		op++;
		int latest_index = adj[t[i] - 'a'][0];
		i++;
		if (i == n)
			break;
		while (1) {
			latest_index = upper_bound(adj[t[i] - 'a'].begin(), adj[t[i] - 'a'].end(), latest_index) - adj[t[i] - 'a'].begin();

			if (latest_index == adj[t[i] - 'a'].size()) {
				break;
			}
			latest_index = adj[t[i] - 'a'][latest_index];
			i++;
			if (i == n) {
				break;
			}
