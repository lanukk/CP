...
void solve(int input)
{
	...
	cin >> n;
	vector<int>ans(n + 1);
	set<pair<int, pair<int, int> >, decltype(comp)* > s(comp);
	s.insert({n, {1, n}});
	int c =  1;
	while (s.size() > 0) {
		pair<int, pair<int, int> > v = *s.begin();
		s.erase(s.begin());
		int elements = v.first;
		int left = v.second.first;
		int right = v.second.second;
		...
		if (pos - left > 0)
			s.insert({pos - 1 - left + 1, {left, pos - 1}});
		if (right - pos > 0)
			s.insert({right - (pos + 1) + 1, {pos + 1, right}});
